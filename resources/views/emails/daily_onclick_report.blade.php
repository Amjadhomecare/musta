...
@php
    ...
    $reportTitle = $p['report_title'] ?? 'Daily Report';
@endphp

# {{ $reportTitle }} — {{ $periodLabel }} — {{ $senderName }}
...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="width:100%;border-collapse:collapse;">
  ...
  <tbody>
    <tr>
      <td style="padding:8px 0;">P4 (HC) — <em>New contracts Today on {{ now('Asia/Dubai')->format('Y-m-d H:i') }}</em></td>
      <td align="right" style="padding:8px 0;">{{ mailr_fmt_num($p4AsNewContracts) }}</td>
    </tr>
    <tr>
      <td style="padding:8px 0;">P5 (Direct Hire) — <em>New contracts Today on {{ now('Asia/Dubai')->format('Y-m-d H:i') }}</em></td>
...
=========================== --}}
@if(!empty($p['cash']))
### {!! mailr_badge('Cash In (Received)', '#0ea5e9') !!}

@component('mail::table')
| Ledger | Amount (AED) |
|:------ | -----------:|
@foreach($p['cash'] as $r)
| {{ $r['ledger'] ?? '' }} | {{ number_format((float)($r['total_received'] ?? 0), 2) }} |
@endforeach
@endcomponent
@endif
...
=========================== --}}
@if(!empty($p['cash_out']))
### {!! mailr_badge('Cash Out (Paid)', '#f59e0b') !!}

@component('mail::table')
| Ledger | Amount (AED) |
|:------ | -----------:|
@foreach($p['cash_out'] as $r)
| {{ $r['ledger'] ?? '' }} | {{ number_format((float)($r['total_paid'] ?? 0), 2) }} |
@endforeach
@endcomponent
@endif
